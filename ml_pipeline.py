...
if __name__ =='__main__':
    ...
    param_grid_xgb = {
                    'select__k': k,
                    'xgb__n_estimators': [1000],     # number of trees
                    'xgb__max_depth': [3, 5, 7, 10],         # tree depth
                    'xgb__learning_rate': [0.1, 0.2, 0.5],   # step size shrinkage
                    'xgb__subsample': [0.2,0.6,0.8],        # row sampling
    ...
    param_grid_ridge = { 
                         'select__k': k,
                         'ridge__alpha': [0.001, 0.01, 0.1],   # Regularization strength
    ...
    #%% training loop
    for k in select_k_lists:
        
        figure_folder = os.path.join(parent_figure_folder,'_'+str(k[0]))
        model_folder = os.path.join(parent_model_folder, '_' + str(k[0]))
        ...
        metrics_path = parent_metrics_path + '_' + str(k[0]) + '.csv'
